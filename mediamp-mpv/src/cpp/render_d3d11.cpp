...
// (png_writer.cpp). Independent of mpv's screenshot pipeline, which cannot convert hwdec
// (d3d11va) frames without zimg. render_mutex_ is only held for the readback; the encode
// works on our own copy.
bool mpv_handle_t::save_surface_png(const char *path) {
    ...
    return write_argb_png(path, width, height, pixels.data());
}
